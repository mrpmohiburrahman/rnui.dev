 ...
 *
 *   * That publication is not guaranteed. Most Submissions are not published,
 *     and a form that merely implies "you are submitting to the catalogue" has
 *     promised something the maintainer never agreed to.
 *   * What happens to the file when it is not published — deleted within 30
 *     days (map decision 11). Without this the honest reading is "you now have
 *     my work indefinitely".
...
export const SUBMISSION_DISCLOSURE_BODY =
  `You are sending rnui.dev a Demo, the name to credit it to, your profile ` +
  `links and a Category. If it is published you will be credited by that name ` +
  `and those links. Publication is not guaranteed — the maintainer decides, and ` +
  `most Submissions are not published. If yours is not, the file is deleted ` +
  `within 30 days. Send only work that is yours to send.`
 ...
 */
export const SUBMISSION_FORM_VERSION = "2026-09-25.1"
