// lib/entry-search.ts
//
// The rule that decides whether an Entry answers a search term. A pure function of
// an Entry and a term, and not a line inside the server action that fetches Entries,
...
// returned zero results when typed. `Misc` returned nothing against 148 Entries.
//
...
// word match across a field boundary — an Entry captioned "…goes split" by an author
// "button Person" would answer to "splitbutton".
const FIELD_SEPARATOR = "\n"

function searchableText(entry: Entry): string {
  return [entry.caption, entry.author, entry.category]
    .join(FIELD_SEPARATOR)
    .toLowerCase()
}

/**
 * True when every word of `term` appears in the Entry's caption, author or Category.
 * Case-insensitive, and each word may come from a different one of the three.
 *
 * An empty or whitespace-only term matches every Entry, which is what makes the
 * unfiltered catalogue the same code path as a filtered one.
 */
export function matchesSearchTerm(entry: Entry, term: string): boolean {
